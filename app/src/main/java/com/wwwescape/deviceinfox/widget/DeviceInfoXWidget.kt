...
import com.wwwescape.deviceinfox.data.settings.settingsRepositoryEntryPoint
import com.wwwescape.deviceinfox.ui.navigation.Destination
import com.wwwescape.deviceinfox.ui.theme.resolveDeviceInfoXColorScheme
 ...
 * [DeviceInfoXWidgetConfigActivity], independent of the app's own theme setting; dynamic color still
 * follows the app's global setting. */
class DeviceInfoXWidget : GlanceAppWidget() {

    override val stateDefinition = PreferencesGlanceStateDefinition

    override suspend fun provideGlance(context: Context, id: GlanceId) {
        val deviceInfo = context.deviceOsRepositoryEntryPoint().collectStatic()
        val battery = context.batteryRepositoryEntryPoint().currentBatteryInfo()
        val storage = context.memoryRepositoryEntryPoint().readStorageVolumes().firstOrNull()
        ...
        // The widget's own theme choice (set via DeviceInfoXWidgetConfigActivity) takes the place
        ...
        val widgetThemeMode = widgetPrefs[WIDGET_THEME_MODE_KEY]?.let { runCatching { ThemeMode.valueOf(it) }.getOrNull() }
            ?: ThemeMode.SYSTEM
        val appSettings = context.settingsRepositoryEntryPoint().settingsFlow.first()
        ...
        val colorScheme = resolveDeviceInfoXColorScheme(
            context = context,
            darkTheme = darkTheme,
        ...
        provideContent {
            GlanceTheme(colors = ColorProviders(colorScheme)) {
                ...
                Column(
                    ...
                    Text(
                        // headlineLarge: 28sp, Normal weight (DeviceInfoXTypography).
