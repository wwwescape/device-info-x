...
interface MessageDao {
     ...
     * [com.wwwescape.deviceinfox.console.data.messaging.MessageRepository.sendText]. Only ever
     * called from within [replacePending]'s transaction — see its doc comment for why calling
     * this on its own as a separate statement is what used to cause a visible flicker. */
    @Query("DELETE FROM messages WHERE id = :id")
    suspend fun deleteById(id: String)

    /** Reconciles a local optimistic placeholder (inserted by [insert] with a client-generated
     * id, [MessageDeliveryState.SENDING]) with the real, server-confirmed row, as a single
     * transaction rather than two separate statements. `observeConversation()`'s Flow — and every
     * other query here — only ever emits once the whole transaction commits, with the final state;
     * without this, deleting the pending row and then upserting the confirmed one under a
     * different id were two separate writes, each triggering its own emission, which a `LazyColumn`
     * keyed on message id rendered as the just-sent message visibly disappearing and reappearing.
     * [pendingId] and [confirmed]'s id are the same value for anything that was never actually
     * pending (every message type except a plain text send today) — the delete is then simply
     * skipped, and this is equivalent to a plain [upsert]. See
     * [com.wwwescape.deviceinfox.console.data.messaging.MessageRepository.applyRemoteMessage]. */
    @Transaction
    suspend fun replacePending(pendingId: String, confirmed: MessageEntity) {
        if (pendingId != confirmed.id) deleteById(pendingId)
        upsert(confirmed)
    }
    ...
    /** The current state of one message — used by `MessageRepository.resendMessage` to read back
     * a [MessageDeliveryState.FAILED] row's own body/replyToId/mediaGroupId before re-sending it. */
    @Query("SELECT * FROM messages WHERE id = :id")
    suspend fun getById(id: String): MessageEntity?

    /** Flips a message that's been stuck in [MessageDeliveryState.SENDING] since before
     * [cutoffEpochMillis] to [MessageDeliveryState.FAILED] — recovers a message whose send never
     * got the chance to mark itself FAILED (the app process died mid-send) from sitting as a
     * silent, permanent "still sending" ghost with no error shown and no way to retry it.
     * [excludedIds] are sends this process itself still has genuinely in flight right now — never
     * swept out from under a coroutine that's still actively working on one of them. See
     * `MessageRepository.sweepStaleSendingMessages`. */
    @Query(
        "UPDATE messages SET deliveryState = 'FAILED' WHERE deliveryState = 'SENDING' " +
            "AND sentAtEpochMillis < :cutoffEpochMillis AND id NOT IN (:excludedIds)",
    )
    suspend fun markStaleSendingAsFailed(cutoffEpochMillis: Long, excludedIds: List<String>)
}
