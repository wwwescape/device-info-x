...
class MediaStorage @Inject constructor(
    ...
    /** Same copy shape as [copyImageToPrivateStorage]/[copyVideoToPrivateStorage], but for a
     * document from `ActivityResultContracts.OpenDocument()`/`OpenMultipleDocuments()` — the
     * single choke point all three features' document pickers copy through (Messages, Calendar,
     * and Safe Locker's `VaultRepository.importDocuments` alike), so this is also the one place
     * the app-wide "documents are PDF-only" rule is actually enforced, not just at the picker's
     * own MIME filter (a content provider's [android.content.ContentResolver.getType] isn't
     * guaranteed to actually match what the picker's filter asked for — this rejects anything
     * that doesn't look like a PDF by either signal, trusting the file's own `.pdf` extension as
     * a fallback in case the resolver misreports or returns null). Unlike the other copy
     * functions, also resolves the picked file's real display name via [resolveDisplayName] —
     * none of them preserve it today, since nothing before this needed to show a filename
     * anywhere. */
    suspend fun copyDocumentToPrivateStorage(sourceUri: Uri): CopiedDocument = withContext(Dispatchers.IO) {
        val mediaDir = File(context.filesDir, MEDIA_DIR_NAME).apply { mkdirs() }
        val resolvedMimeType = context.contentResolver.getType(sourceUri)
        val originalFilename = resolveDisplayName(sourceUri)
        val looksLikePdf = resolvedMimeType == "application/pdf" ||
            originalFilename?.endsWith(".pdf", ignoreCase = true) == true
        require(looksLikePdf) { "Only PDF documents are supported" }
        val destFile = File(mediaDir, "${UUID.randomUUID()}.pdf")
    ...
    /** The one deliberate exception to every attachment type living in app-private storage: this
     * writes into wherever the user picks via the system "Save As" document picker
     * (`ActivityResultContracts.CreateDocument`, launched from the UI layer) — the user
     * explicitly asking to make a copy of the file visible outside the app again. Shared by
     * Messages, Calendar, and Safe Locker (moved here from what used to be Vault-only
     * `VaultStorage.exportToDocument`, since it never actually touched any Vault-specific
     * state). */
    suspend fun exportToDocument(sourceFilePath: String, destinationUri: Uri) = withContext(Dispatchers.IO) {
        context.contentResolver.openOutputStream(destinationUri)?.use { output ->
            File(sourceFilePath).inputStream().use { input -> input.copyTo(output) }
        }
    }
    ...
    /** Downloads a picked GIF's full bytes through this server's own `/gifs/proxy` route (never
     * Klipy's CDN directly, see [GifsApi]'s doc comment) into the same private-storage shape
     * [copyImageToPrivateStorage] produces for a gallery pick — so the caller can hand the
     * resulting file straight to `MessageRepository.sendImage` as an ordinary local `Uri`, with no
     * dedicated "send a GIF" path needed anywhere downstream. */
    suspend fun downloadGifToMediaDir(proxyPath: String): DownloadedMedia = withContext(Dispatchers.IO) {
        val mediaDir = File(context.filesDir, MEDIA_DIR_NAME).apply { mkdirs() }
        val body = consoleApiCall { gifsApi.proxy(proxyPath) }
        val mimeType = body.contentType()?.toString()?.substringBefore(';')?.trim() ?: "image/gif"
        val extension = mimeType.substringAfter('/', "gif")
        val destFile = File(mediaDir, "${UUID.randomUUID()}.$extension")
    ...
    /** Writes an in-memory [bitmap] (e.g. a captured Calendar stats chart) to the same private
     * storage as every other outgoing image, as a PNG, so the caller can hand it to
     * `MessageRepository.sendImage` as an ordinary local `Uri` — same shape as
     * [downloadStickerToMediaDir]/[downloadGifToMediaDir]. The caller deletes the file afterwards
     * (`sendImage` copies its own private duplicate). */
    suspend fun saveBitmapToMediaDir(bitmap: Bitmap): DownloadedMedia = withContext(Dispatchers.IO) {
        val mediaDir = File(context.filesDir, MEDIA_DIR_NAME).apply { mkdirs() }
        val destFile = File(mediaDir, "${UUID.randomUUID()}.png")
        destFile.outputStream().use { output -> bitmap.compress(Bitmap.CompressFormat.PNG, 100, output) }
        DownloadedMedia(filePath = destFile.absolutePath, mimeType = "image/png")
    }

    /** Downloads a server-hosted sticker's PNG into the same private-storage shape
     * [downloadGifToMediaDir] produces, so the caller can hand it straight to
     * `MessageRepository.sendImage` as an ordinary local `Uri` — a sticker is just an image
     * message whose bytes happen to be a transparent PNG (bytes are copied verbatim, never
     * re-encoded, so transparency survives). */
    suspend fun downloadStickerToMediaDir(tier: AssetTier, id: String): DownloadedMedia =
        withContext(Dispatchers.IO) {
            val mediaDir = File(context.filesDir, MEDIA_DIR_NAME).apply { mkdirs() }
            val body = consoleApiCall { assetsApi.sticker(tier.path, id) }
            val destFile = File(mediaDir, "${UUID.randomUUID()}.png")
