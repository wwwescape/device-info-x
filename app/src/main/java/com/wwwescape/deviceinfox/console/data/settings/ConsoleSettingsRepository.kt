...
class ConsoleSettingsRepository @Inject constructor(
    ...
    private val historyFullyBackfilledKey = booleanPreferencesKey("history_fully_backfilled")

    /** True once [com.wwwescape.deviceinfox.console.data.messaging.MessageRepository]'s background
     * history backfill has walked all the way back to the start of the conversation — after that,
     * Room holds the full message history rather than only whatever's been scrolled to, so backfill
     * is skipped on every later app start instead of re-verifying it every time. Defaults to false
     * for every existing install too, same as a fresh pairing — everyone backfills exactly once. */
    val isHistoryFullyBackfilled: Flow<Boolean> = context.consoleSettingsDataStore.data.map { prefs ->
        prefs[historyFullyBackfilledKey] ?: false
    }

    suspend fun setHistoryFullyBackfilled(complete: Boolean) {
        context.consoleSettingsDataStore.edit { prefs -> prefs[historyFullyBackfilledKey] = complete }
    }

    private val whatsNewSeenTagsCacheKey = stringSetPreferencesKey("whats_new_seen_tags_cache")

    /** Same-device fallback for [com.wwwescape.deviceinfox.console.data.whatsnew.WhatsNewRepository]'s
     * server-fetched seen-tag set — that repository's own `refresh()` seeds from this before the
     * network call and overwrites it on every successful fetch, so it's never the source of truth
     * (a fresh install/reinstall still correctly starts empty and needs the server), only a guard
     * against a server outage wrongly making already-dismissed entries look never-seen. See
     * TODOS.md's "What's New / Guided Tours: stop reappearing when the server is unreachable" item
     * for why this exists — `Set<String>` order doesn't matter here (unlike [recentEmojis] above),
     * so `stringSetPreferencesKey` is the right fit, no delimited-string workaround needed. */
    val whatsNewSeenTagsCache: Flow<Set<String>> = context.consoleSettingsDataStore.data.map { prefs ->
        prefs[whatsNewSeenTagsCacheKey] ?: emptySet()
    }

    suspend fun setWhatsNewSeenTagsCache(tags: Set<String>) {
        context.consoleSettingsDataStore.edit { prefs -> prefs[whatsNewSeenTagsCacheKey] = tags }
    }

    private val tourSeenKeysCacheKey = stringSetPreferencesKey("tour_seen_keys_cache")

    /** Same treatment as [whatsNewSeenTagsCache], for
     * [com.wwwescape.deviceinfox.console.data.tour.FeatureTourRepository]'s seen-tour-key set. */
    val tourSeenKeysCache: Flow<Set<String>> = context.consoleSettingsDataStore.data.map { prefs ->
        prefs[tourSeenKeysCacheKey] ?: emptySet()
    }

    suspend fun setTourSeenKeysCache(keys: Set<String>) {
        context.consoleSettingsDataStore.edit { prefs -> prefs[tourSeenKeysCacheKey] = keys }
    }
