 ...
 * [com.wwwescape.deviceinfox.console.data.messaging.MediaStorage]'s doc comment.
 *
 * Export is the one deliberate exception: it writes into wherever the user picks via the
 * system "Save As" document picker ([android.provider.DocumentsContract] /
 * `ActivityResultContracts.CreateDocument`, launched from the UI layer). That needs no storage
 * permission on any API level — the returned [Uri] already carries write access — and is the
 * user explicitly asking to make an item visible outside the vault again, which is exactly
 * what "export" in the design spec means.
 */
@Singleton
