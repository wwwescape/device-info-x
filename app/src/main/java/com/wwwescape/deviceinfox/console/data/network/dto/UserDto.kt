...
/** Mirrors `InsightOut` (`app/schemas/insight.py`) — one "DIX AI" insight's server-authored copy,
 * per [com.wwwescape.deviceinfox.console.data.insights.InsightsRepository]. Unlike
 * [SeenWhatsNewDto], the text itself is fetched here rather than living client-side — DIX AI
 * insights are manually written server-side via the CLI, not shipped in the app. */
@Serializable
...
/** Mirrors `UnseenInsightsOut` (`GET /users/me/unseen-insights`). */
@Serializable
data class UnseenInsightsDto(
