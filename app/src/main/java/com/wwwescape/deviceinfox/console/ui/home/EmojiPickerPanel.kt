...
// One fixed content height shared by the Emoji, GIF and Reactions tabs (excluding the tab strip
// above them), so switching tabs never resizes the composer's picker panel. It's the tallest of the
// three tabs' natural heights: search field (~64dp) + checkbox row (48dp) + a 220dp grid = 332dp
// (the GIF and Reactions tabs; the Emoji tab was 324dp — its grid just gets the extra few dp).
internal val PICKER_CONTENT_HEIGHT = 332.dp
private val EMOJI_TAB_STRIP_HEIGHT = 40.dp

private enum class PickerTab { EMOJI, GIF, REACTIONS }
 ...
 * the icon-only Emoji/GIF/Reactions tab strip (above the search field, distinct from [EmojiGrid]'s
 * own bottom category-jump strip, which stays scoped to the Emoji tab) and dispatches to
 * [EmojiGrid]/[GifPickerPanel]/[ReactionsPanel] accordingly — all three tabs share this one
...
fun EmojiPickerPanel(
    ...
    onLoadMatureEmojis: suspend () -> List<String> = { emptyList() },
) {
