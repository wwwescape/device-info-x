 ...
 * the title needs the count baked in. [isPending] mirrors the single-item dialog's own — added
 * after `bulkDelete()` was found to bypass the pending-state guard entirely (it now routes
 * through the same `launchCatching`/`_isPending` machinery every other Vault action uses). */
@Composable
