...
data class ConsoleMessage(
    ...
    /** Null for every non-location message — same "inferred from presence, not a stored type
     * field" shape [attachment]/[voiceNote]/[poll] already use. */
    val location: MessageLocation?,
    /** Shared by every sibling message in one multi-select photo/video "album" send — null means
     * this message wasn't sent as part of a multi-select batch. See `HomeScreen.kt`'s
     * `groupMessagesForDisplay` for how this drives the grouped grid bubble. */
    val mediaGroupId: String? = null,
)

/** [bodyPreview] null doesn't by itself mean "deleted" — a media message (image/video/voice
 * note/document/poll/location) has no text body either, and [isDeleted]/[kind] are what the UI
 * needs to tell those null-body cases apart (see `HomeScreen.kt`'s reply-preview `Text`). */
...
/** What kind of non-text content the replied-to message carries — null means it's a plain text
 * message (or deleted; [ReplyPreview.isDeleted] is checked ahead of this). [DOCUMENT] is broken
 * out from [MEDIA] (image/video/voice note) since it gets its own "[Document]" preview string
 * rather than the generic "[Media]" one. */
enum class ReplyPreviewKind { MEDIA, DOCUMENT, POLL, LOCATION }
...
/** A one-time GPS snapshot captured at send time — distinct from Live Location's continuous/timed
 * sharing, which has its own separate data model entirely. */
