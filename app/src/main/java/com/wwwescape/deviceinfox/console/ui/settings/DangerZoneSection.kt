 ...
 *
 * No card-level title — the "Danger Zone" [CategoryHeader] right above this in [ConsoleSettingsScreen]
...
@Composable
fun DangerZoneSection(onDeleteAllDataClick: () -> Unit, onDestroyAllDataClick: () -> Unit) {
    SettingsSectionCard(title = null) {
        DangerZoneRow(label = stringResource(R.string.console_settings_delete_all_data_action), onClick = onDeleteAllDataClick)
        HorizontalDivider(color = MaterialTheme.colorScheme.outlineVariant.copy(alpha = 0.4f))
        DangerZoneRow(label = stringResource(R.string.console_settings_destroy_all_data_action), onClick = onDestroyAllDataClick)
    }
}

@Composable
private fun DangerZoneRow(label: String, onClick: () -> Unit) {
    Row(
