...
/** Multi-attachment "album" grouping — a plain nullable tag column, null for every pre-existing
 * row (matching [MIGRATION_26_27]'s own "starts blank" treatment): a client-generated id shared
 * by every sibling message in one multi-select gallery send, so the chat list can visually cluster
 * them into one grid bubble. See [MIGRATION_26_27]'s own doc comment for the general shape. */
val MIGRATION_28_29 = object : Migration(28, 29) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE messages ADD COLUMN mediaGroupId TEXT")
    }
}
