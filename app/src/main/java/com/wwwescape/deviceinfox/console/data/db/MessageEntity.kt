...
data class MessageEntity(
    ...
    // Shared by every sibling message in a multi-select photo/video "album" send (one id per
    // batch, generated client-side) — a plain tag, not a relation, so no FK. Null on every
    // message that isn't part of a multi-select send, same "unused on every other type" treatment
    // as the poll*/location* fields above.
