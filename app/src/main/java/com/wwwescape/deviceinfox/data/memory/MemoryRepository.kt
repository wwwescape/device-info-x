...
class MemoryRepository @Inject constructor(
    ...
    fun liveStats(intervalMillis: Long = 2_000L): Flow<MemoryLiveStats> = flow {
        while (true) {
            emit(MemoryLiveStats(ram = readRamInfo(), storageVolumes = readStorageVolumes()))
    ...
    fun readStorageVolumes(): List<StorageVolumeInfo> {
...
fun Context.memoryRepositoryEntryPoint(): MemoryRepository =
    EntryPointAccessors.fromApplication(applicationContext, MemoryRepositoryEntryPoint::class.java)
        .memoryRepository()
